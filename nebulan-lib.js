(function ($) {
    KWNebulanLibrary = {
            ...
            NoisePerlin: function (grid_size) {
                ...
                self.shufflePermutation = function () {
                    var self = this;
            ...
            Frame: function (frame_size) {
                ...
                self.add = function (aFrame) {
                    var self = this;
                    for (var i = self.width - 1; i >= 0; i--)
                        for (var j = self.height - 1; j >= 0; j--)
                            self.frame[i][j] = self.frame[i][j].add(aFrame.frame[i][j]);
                };
                self.multiplyByScalar = function (value) {
                    var self = this;
                    for (var i = self.width - 1; i >= 0; i--)
                        for (var j = self.height - 1; j >= 0; j--)
                            self.frame[i][j] = self.frame[i][j].multiply(value);
                };
                self.subtract = function (aFrame) {
                    var self = this;
                    for (var i = self.width - 1; i >= 0; i--)
                        for (var j = self.height - 1; j >= 0; j--)
                            self.frame[i][j] = self.frame[i][j].subtract(aFrame.frame[i][j]);
                };
